    ...
    @foreach($item->images as $image)
        @if($image != null)
            <div class="imgUp mb-3">
                <label class="label w-75" data-toggle="tooltip" title="Изменить изображение">
                    <img class="rounded w-100 avatar" data-session-id=""  src="@isset($item){{ asset($item->getImages($loop->index,'images',($default_img ?? '/admin/img/png/no_avatar.png'))) }}@else {{ asset($default_img ?? '/admin/img/png/no_avatar.png') }} @endisset" >
...
@if(isset($addPhoto) and $addPhoto === true)
    <div class="text-primary imgAddNew" data-tooltip="tooltip" data-html="true" title="Добавить изображение">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                <div class="img-container img-fluid w-100">
                    <img id="image" src="@isset($item){{ asset($item->getImages(0,'images',($default_img ?? '/admin/img/png/no_avatar.png'))) }}@else {{ asset($default_img ?? '/admin/img/png/no_avatar.png') }} @endisset" class="w-100" width="100%">
                </div>
            </div>
