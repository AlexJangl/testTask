...
trait AddEdit
{
    ...
    {
        return strtotime($this->$data_at) ? $this->$data_at->format('d.m.Y') . $separator . $this->$data_at->format('H:i') : null;
    }

    public function code_status()
    {
        return '<a class="btn btn-sm btn-block btn-outline-'. ($this->status->color ?? 'primary') . ' disabled">' . $this->status->title . '</a>';
    }
